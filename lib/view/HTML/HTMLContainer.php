 ...
 * 
 * This class manages the javascript event handlers for most tags, which will be automatically applied to the element
 * when rendered. This will only bind to this particular element, if you need jquery multi matchers,
 * you should use the jQuery class's static methods. Only the common events are specified here, this does
 * not stop you from registering another type of event with the generic onEvent method.
 */
abstract class HTMLContainer extends HTMLView {
	...
	/**
	 * Unlike $events, this is used when pulling up the events, to store the
	 * aggregated event chain. It should only be used by HTMLView when
	 * extracting.
	 */
	private $aggregateEvents = null;

	final public function getAggregateEvents() {
		if ($this->aggregateEvents === null) {
			$this->aggregateEvents = array();
		}
		return $this->aggregateEvents;
	}

	protected function consolidate(HTMLView $subView) {
		parent::consolidate($subView);
		if ($subView instanceof HTMLContainer) {
			if ($subView->events !== null) {
				if ($this->aggregateEvents === null) {
					$this->aggregateEvents = array();
				}
				$this->aggregateEvents = array_merge($this->aggregateEvents, $subView->getEvents());
			}
		}
	}

	/**
	 * This is an array of [eventType][handler]. Before it is returned however,
	 * it is changed into [id][eventType][handler], so it can be rendered by the
	 * page mananger appropriately, and an ID can lazily be assigned.
	 * @var type 
	 */
	private $events = null;
	...
	final protected function getEvents() {
		...
		return array($id => $this->events);
	}

	public function onEvent($eventName, $javascript) {
		if ($this->events === null) {
			$this->events = array();
		}
		if ($javascript instanceof Structure) {
			$javascript = (string) $javascript; //This will render the javascript, so now we have text
		}
		$this->events[$eventName][] = $javascript;
	}

	public function onClick($javascript) {
		$this->onEvent("click", $javascript);
	}
	
	public function onDblClick($javascript){
		$this->onEvent("dblclick", $javascript);
	}
	
	public function onMouseOver($javascript){
		$this->onEvent("mouseover", $javascript);
	}
	
	public function onMouseOut($javascript){
		$this->onEvent("mouseout", $javascript);
	}
	
	public function onMouseMove($javascript){
		$this->onEvent("mousemove", $javascript);
	}
	...
	/**
	 * Returns this element's ID, or null if one isn't set.
	 * @return type 
	 */
